...
#    add-apt-repository -y ppa:remnux/stable >> $HOME/oafe-install.log 2>&1 || return 1
	...
	echoinfo "Enabling Elasticsearch repository"
	wget -qO - https://packages.elastic.co/GPG-KEY-elasticsearch | sudo apt-key add -  >> $HOME/oafe-install.log || return 1
	echo "deb http://packages.elastic.co/elasticsearch/4.4/debian stable main" | sudo tee -a /etc/apt/sources.list.d/elasticsearch-2.x.list >> $HOME/oafe-install.log || return 1

	echoinfo "Enabling Kibana repository"
	echo "deb http://packages.elastic.co/kibana/4.4/debian stable main" | sudo tee -a /etc/apt/sources.list.d/kibana-4.4.x.list >> $HOME/oafe-install.log || return 1

	echoinfo "Enabling Logstash repository"
	echo 'deb http://packages.elastic.co/logstash/2.2/debian stable main' | sudo tee /etc/apt/sources.list.d/logstash-2.2.x.list >> $HOME/oafe-install.log || return 1
...
echoinfo "Installing Elasticsearch, Kibana, Logstash, and Graylog as services"
    #if [ ! -d /opt/oafe/elk ]; then
    #    mkdir -p /opt/oafe/elk
    #    chown cfi:cfi /opt/oafe/elk
    #    chmod -R 775 /opt/oafe/elk
    #    chmod -R g+s /opt/oafe/elk
    #fi
    ...
    #wget -O /opt/oafe/elk/elasticsearch-2.4.1.deb https://download.elastic.co/elasticsearch/release/org/elasticsearch/distribution/deb/elasticsearch/2.4.1/elasticsearch-2.4.1.deb
    #wget -O /opt/oafe/elk/logstash-2.4.0_all.deb https://download.elastic.co/logstash/logstash/packages/debian/logstash-2.4.0_all.deb
    #wget -O /opt/oafe/elk/kibana-4.6.1-amd64.deb https://download.elastic.co/kibana/kibana/kibana-4.6.1-amd64.deb
...
echoinfo "Installing Cuckoo Sandbox"
        ...
        vboxmanage hostonlyif ipconfig vboxnet0 --ip 192.168.56.1 >> $HOME/oafe-install.log || return 1
        iptables -A FORWARD -o eth0 -i vboxnet0 -s 192.168.56.0/24 -m conntrack --ctstate NEW -j ACCEPT >> $HOME/oafe-install.log || return 1
        iptables -A FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT >> $HOME/oafe-install.log || return 1
        iptables -A POSTROUTING -t nat -j MASQUERADE >> $HOME/oafe-install.log || return 1
...
echoinfo "Installing Moloch DPI"
		...
		mkdir -p /opt/oafe/moloch
		chown $SUDO_USER:$SUDO_USER /opt/oafe/moloch
		chmod -R 775 /opt/oafe/moloch
		chmod -R g+s /opt/oafe/moloch
	fi
        git clone https://github.com/aol/moloch /opt/oafe/moloch >> $HOME/oafe-install.log 2>&1
        cp -f /opt/oafe/oafeubuntu/molochinstaller/easybutton-config.sh /opt/oafe/moloch/easybutton-config.sh
        cp -f /opt/oafe/oafeubuntu/molochinstaller/easybutton-singlehost.sh /opt/oafe/moloch/easybutton-singlehost.sh
...
echoinfo "Install Kansa Files for Threat Hunting"
    ...
    cp /opt/oafe/conf/logstash/ingest/logstash_kansa_ingest.conf /etc/logstash/conf.d/logstash_kansa_ingest.conf
...
echoinfo "Downloading and installing Cuckoo Analysis VMs...this could take a while ~7GB"
        ...
        wget https://www.dropbox.com/s/tl09k29l9globs6/Windows7x64Sandbox1.ova?dl=0 -O /opt/oafe/VMs/Windows7x64Sandbox1.ova | tee -a "$HOME/oafe-install.log"
        wget https://www.dropbox.com/s/0d868pk0kzfueky/WindowsXPSP3Sandbox1.ova?dl=0 -O /opt/oafe/VMs/WindowsXPSP3Sandbox1.ova | tee -a "$HOME/oafe-install.log"
        ...
        sudo -u cfi vboxmanage import /opt/oafe/VMs/Windows7x64Sandbox1.ova
        sudo -u cfi vboxmanage import /opt/oafe/VMs/WindowsXPSP3Sandbox1.ova
}
